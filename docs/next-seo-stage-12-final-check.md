...
Все 16 страниц экскурсий из `next/src/data/tours.ts` проверены по slug: каждая отдает 200, содержит H1 и путь к заказу.
...
- В Next и старом Vite-конфиге исправлен email на `bogdasovanton83@gmail.com`.
...
- В `next/app` и `next/src` не найдено длинных тире `—` и `–`.
- В `next/app` и `next/src` не найдено явных признаков битой кодировки: `вЂ`, `�`, mojibake-паттерны `Р...`/`С...`.
